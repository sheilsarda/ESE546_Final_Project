...
def lets_plot_baby(tasks_list, meta_rewards): 
    ...
    for i in range(len(tasks_list)):    
        ...
        plt.title("Run Number " + str(i) + ", Task " + str(tasks_list[i]) + " No Early Stopping")
        plt.grid()
        plt.ylabel("Reward")
        plt.xlabel("Episode")
        ...
        plt.savefig(out_dir + "Task_" + str(tasks_list[i]) + "_run_" + str(i) + "_no_es" + ".png")

    
    # all plot
    plt.figure(figsize=(20,10))
    for i in range(len(tasks_list)):
        strLabel = "Run Number " + str(i) + ", Task " + str(tasks_list[i] + " No Early Stopping")
        plt.plot(np.arange(1, len(meta_rewards[i])+1), meta_rewards[i], label=strLabel)

    plt.grid() 
    plt.title("All Tasks, No Early Stopping")
    plt.ylabel("Reward")
    plt.xlabel("Episode")
    plt.legend()
    ...
    plt.savefig(out_dir + "Task_all_no_es.png")
